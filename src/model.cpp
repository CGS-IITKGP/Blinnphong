...
unsigned int TextureFromFile(const char* path, const std::string& directory)
{
    ...
    {
        std::cout << "[TEXTURE] Texture failed to load at path: " << filename << std::endl;
        stbi_image_free(data);
    }
...
void Model::loadModel(std::string path)
{
    ...
    std::cout << "[MODEL] Model loaded successfully. Starting to process nodes..." << std::endl;
...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName) {
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
        ...
        if (texPath.find("<UDIM>") != std::string::npos)
        {
            std::cout << "[TEXTURE] Skipping UDIM texture: "<< texPath<< std::endl;
            continue;
        }
